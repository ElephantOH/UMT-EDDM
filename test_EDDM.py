...
from backbones.eddm import EDDM
...
def sample_from_model(coefficients, mapping_model, x_T, source, args):
    ...
    with torch.no_grad():
        for t in reversed(range(args.vp_t)):
            ...
            latent_z = torch.zeros(x_t.size(0), args.z_emb_dim, device=x_t.device) if args.sample_fixed else torch.randn(x_t.size(0), args.z_emb_dim, device=x_t.device)
            h_t, _ = mapping_model(torch.cat((x_t, source),axis=1), t_module, latent_z)
...
def sample_and_test(args):
    ...
    test_dataset = GetDataset("test", args.input_path, args.source, args.target, dim=args.input_channels, normed=args.normed, unaligned=args.use_unaligned)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=0)

    mapping_model = EDDM(args).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t_emb_channels', nargs='+', type=int, default=[256, 256, 256])
    ...
    parser.add_argument('--blur_list', nargs='+', type=float, default=[0., 0., 0.5, 1.])
    parser.add_argument('--use_reg', action='store_true', default=True)
    parser.add_argument('--use_unaligned', action='store_true', default=False)
